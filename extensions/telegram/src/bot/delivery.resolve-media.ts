...
import { renderAnimatedStickerFrame, renderVideoStickerFrame } from "../sticker-render.js";
...
> {
  ...
  try {
    ...
    const downloaded = await downloadAndSaveTelegramFile({
    ...
    // Render animated/video stickers to PNG so vision models can process them
    let saved: { path: string; contentType?: string } = downloaded;
    if (sticker.is_animated || sticker.is_video) {
      const renderFn = sticker.is_video ? renderVideoStickerFrame : renderAnimatedStickerFrame;
      const pngBuf = await renderFn(downloaded.path);
      if (pngBuf) {
        const rendered = await saveMediaBuffer(pngBuf, "image/png", "inbound", maxBytes, "sticker.png");
        saved = { path: rendered.path, contentType: "image/png" };
      } else {
        logVerbose("telegram: sticker rendering failed, sticker will be skipped");
        return null;
      }
    }
